...
const CLIENT_MSG_TYPE_KEEPALIVE = 185;
const SERVER_MSG_TYPE_DISCONNECT_NOTIFY = 186;
...
export default class RFB extends EventTargetMixin {
  constructor(target, touchInput, urlOrChannel, options, videoCodecs, isPrimaryDisplay) {
    ...
    this._preserveLastActiveAtOnConnect =
      !!options.preserveLastActiveAtOnConnect;
    ...
    this._connectionID = window.location.href
      .split("?")[0]
      .match(/^(.+)(\/)/)[0];
    this._screenFlags = 0;
    ...
    try {
      this._display = new Display(
        this._canvas,
        this,
        this._isPrimaryDisplay,
        this._videoRenderingMode,
      );
    } catch (exc) {
    ...
    this._decoders[encodings.encodingKasmVideo] = new KasmVideoDecoder(
      this,
      this._display,
    );
  ...
  get pointerRelative() {
    return this._directMouseEnabled;
  }
  ...
  set frameRate(value) {
    if (!isValidFrameRate(value)) {
      Log.Error(
        `frame rate must be an integer between ${FRAME_RATE_MIN} and ${FRAME_RATE_MAX}`,
      );
      return;
    }
  ...
  set streamMode(value) {
    if (value !== this._streamMode) {
      ...
      this._pendingApplyEncodingChanges = true;
      if (this._display) {
        this._display.preferSoftwareDecode =
          value === encodings.pseudoEncodingStreamingModeAVCNVENC;
      }
    }
  }
  ...
  applyScreenPlan(screenPlan) {
    if (this._isPrimaryDisplay) {
      ...
      if (numScreensFound > fullPlan.screens.length) {
        throw new Error(
          "Screen plan contained more screens then there are registered.",
        );
      } else if (numScreensFound < fullPlan.screens.length) {
        throw new Error(
          "Screen plan contained fewer screens then there are registered.",
        );
      }
      ...
      if (changes) {
        ...
        for (let i = 0; i < screenPlan.screens.length; i++) {
          for (let z = 1; z < fullPlan.screens.length; z++) {
            if (
              screenPlan.screens[i].screenID == fullPlan.screens[z].screenID
            ) {
  ...
  updateConnectionSettings() {
    if (this._rfbConnectionState === "connected" && this._isPrimaryDisplay) {
      if (this._pendingApplyVideoRes) {
        RFB.messages.setMaxVideoResolution(
          this._sock,
      ...
      if (this._pendingApplyResolutionChange) {
        ...
        if (
          this._resizeSession ||
          (this._forcedResolutionX && this._forcedResolutionY)
        ) {
        ...
        if (this.clipboardBinary) {
          ...
          if (navigator.clipboard && navigator.clipboard.readText) {
            try {
              (() => {
                try {
                  return navigator.clipboard.readText() || Promise.reject();
                } catch (e) {
  ...
  _changeTransitConnectionState(value) {
    Log.Info(
      "Transit state change from " +
        this._transitConnectionState.toString() +
        " to " +
        value.toString(),
    );
  ...
  _connect() {
    ...
    this._canvas.addEventListener(
      "focus",
      this._eventHandlers.handleFocusChange,
    );
    ...
    document.addEventListener(
      "visibilitychange",
      this._eventHandlers.handleVisibilityChange,
    );
    ...
    if (isIOS()) {
      this._canvas.addEventListener(
        "touchend",
        this._eventHandlers.updateHiddenKeyboard,
      );
    }
    ...
    if (document.onpointerlockchange !== undefined) {
      document.addEventListener(
        "pointerlockchange",
        this._eventHandlers.handlePointerLockChange,
        false,
      );
      document.addEventListener(
        "pointerlockerror",
        this._eventHandlers.handlePointerLockError,
        false,
      );
    ...
    if (typeof RTCPeerConnection !== "undefined" && this._isPrimaryDisplay) {
      ...
      this._udpChannel.onmessage = function (e) {
        ...
        const hash = parseInt(
          u8[12] + (u8[13] << 8) + (u8[14] << 16) + (u8[15] << 24),
          10,
        );
        // TODO: check the hash. It's the low 32 bits of XXH64, seed 0
        const frame_id = parseInt(
          u8[16] + (u8[17] << 8) + (u8[18] << 16) + (u8[19] << 24),
          10,
        );
  ...
  _disconnect() {
    ...
    if (this._isPrimaryDisplay) {
      ...
      if (this._primaryDisplayChannel) {
        ...
        this._primaryDisplayChannel.removeEventListener(
          "message",
          this._handleSecondaryDisplayMessage,
        );
  ...
  _updateClip() {
    ...
    if (newClip) {
      ...
      this._display.viewportChangeSize(
        size.screens[0].serverWidth,
        size.screens[0].serverHeight,
      );
      this._fixScrollbars();
    }
  }
  ...
  _updateScale() {
    if (!this._scaleViewport) {
      ...
      const size = this._screenSize();
      this._display.autoscale(
        size.screens[0].width,
        size.screens[0].height,
        size.screens[0].scale,
      );
    }
    this._fixScrollbars();
  }
  ...
  _requestRemoteResize() {
    ...
    if (this._isPrimaryDisplay) {
      ...
      for (let i = 0; i < this._display.screens.length; i++) {
        this._display.applyServerResolution(
          0,
          0,
          this._display.screens[i].screenIndex,
        );
      }
      ...
      let details = null;
      const initialAutoPlacementValue =
        window.localStorage.getItem("autoPlacement");
    ...
    if (this._isPrimaryDisplay) {
      ...
      switch (event.data.eventType) {
        case "register":
          ...
          let screenIndex = this._display.addScreen(
            event.data.screenID,
            event.data.width,
            event.data.height,
          ...
          this.dispatchEvent(
            new CustomEvent("screenregistered", { detail: details }),
          );
          Log.Info(
            `Secondary monitor (${event.data.screenID}) has been registered.`,
          );
          break;
        case "reattach":
          let changes = this._display.addScreen(
            event.data.screenID,
            event.data.width,
            event.data.height,
            event.data.pixelRatio,
          ...
          this.dispatchEvent(new CustomEvent("screenregistered", {}));
          Log.Info(
            `Secondary monitor (${event.data.screenID}) has been reattached.`,
          );
          break;
        case "unregister":
          if (this._display.removeScreen(event.data.screenID)) {
            this.dispatchEvent(new CustomEvent("screenregistered", {}));
            Log.Info(
              `Secondary monitor (${event.data.screenID}) has been removed.`,
            );
            const size = this._screenSize();
            RFB.messages.setDesktopSize(this._sock, size, this._screenFlags);
            this._sendEncodings();
  ...
  _registerSecondaryDisplay(currentScreen = false, details = null) {
    if (!this._isPrimaryDisplay) {
      ...
      const windowId = new URLSearchParams(document.location.search).get(
        "windowId",
      );
  ...
  _handleMouseOut(ev) {
    if (
      ...
      if (this._display.screens[0].pixelRatio !== window.devicePixelRatio) {
        Log.Debug(
          "Window moved to another screen with different pixel ratio, sending resize request.",
        );
    ...
    if (this._pointerLock && !this._pointerRelativeEnabled) {
      let max_w =
        this._display.scale === 1
          ? this._fbWidth
          : this._fbWidth * this._display.scale;
      let max_h =
        this._display.scale === 1
          ? this._fbHeight
          : this._fbHeight * this._display.scale;
      pos = {
    ...
    switch (ev.type) {
      case "mousedown":
        ...
        if (
          isMac() &&
          ev.metaKey &&
          (this._keyboard._keyDownList["MetaLeft"] ||
            this._keyboard._keyDownList["MetaRight"])
        ) {
          this._keyboard._sendKeyEvent(
            this._keyboard._keyDownList["MetaLeft"],
            "MetaLeft",
            false,
          );
        ...
        if (this._isPrimaryDisplay) {
          this._handleMouseButton(
            ...
            pos.y,
            true,
            xvncButtonToMask(mappedButton),
          );
        } else {
          this._proxyRFBMessage("mousedown", [
            pos.x,
            pos.y,
            xvncButtonToMask(mappedButton),
          ]);
      ...
      case "mousemove":
        ...
        if (this._isPrimaryDisplay) {
          ...
          this._proxyRFBMessage("mousemove", [
            pos.x,
            pos.y,
            ev.buttons > 0,
            this._sendLeftClickonNextMove,
          ]);
          this._sendLeftClickonNextMove = false;
        }
        break;
  ...
  _handleMouseMove(x, y, down, simulated = false) {
    ...
    if (
      this._display.screens.length > 1 &&
      this._mouseButtonMask !== 0 &&
      !down &&
      !simulated &&
      isDesktop()
      ...
      this._mouseButtonMask = 0;
      Log.Debug(
        "Mouse event button down mismatch with current mask, resetting mask to 0.",
      );
    }
  ...
  _handlePointerLockChange(env) {
    ...
    this.dispatchEvent(
      new CustomEvent("inputlock", { detail: { pointer: this._pointerLock } }),
    );
  }
  ...
  _handlePointerLockError() {
    ...
    this.dispatchEvent(
      new CustomEvent("inputlockerror", {
  ...
  _sendMouse(x, y, mask) {
    ...
    if (this._pointerLock && this._directMouseEnabled) {
      ...
      RFB.messages.pointerEventClamped(
        this._sock,
  ...
  _handleWheel(ev) {
    ...
    if (ev.ctrlKey && !this._keyboard._keyDownList["ControlLeft"]) {
      ...
      this._watchForPinchAndZoom =
        this._watchForPinchAndZoom ||
        setInterval(() => {
          const timeSinceLastPinchAndZoom =
            +new Date() - this._mouseLastPinchAndZoomTime;
  ...
  _sendEncodings() {
    ...
    encs.push(
      encodings.pseudoEncodingJpegVideoQualityLevel0 + this.jpegVideoQuality,
    );
    encs.push(
      encodings.pseudoEncodingWebpVideoQualityLevel0 + this.webpVideoQuality,
    );
    encs.push(encodings.pseudoEncodingTreatLosslessLevel0 + this.treatLossless);
    encs.push(
      encodings.pseudoEncodingDynamicQualityMinLevel0 + this.dynamicQualityMin,
    );
    encs.push(
      encodings.pseudoEncodingDynamicQualityMaxLevel0 + this.dynamicQualityMax,
    );
    ...
    encs.push(
      encodings.pseudoEncodingVideoOutTimeLevel1 + this.videoOutTime - 1,
    );
    ...
    encs.push(
      encodings.pseudoEncodingStreamingVideoQualityLevel0 +
        this.videoStreamQuality,
    );
  ...
  _handleBinaryClipboard() {
    ...
    Log.Debug(
      "Started clipbooard processing with Client sockjs buffer size " +
        this._sock.rQlen,
    );
    ...
    for (let i = 0; i < num; i++) {
      ...
      if (
        this._sock.rQwait("Binary Clipboard data", Math.abs(len), buffByteLen)
      ) {
        return false;
      }
      ...
      switch (mime) {
        ...
        case "text/plain":
          ...
          if (mime == "text/plain") {
            ...
            Log.Debug(
              "Plain text clipboard recieved and placed in text element, size: " +
                textdata.length,
            );
            this.dispatchEvent(
              new CustomEvent("clipboard", { detail: { text: textdata } }),
    ...
    Log.Debug(
      "Finished processing binary clipboard with client sockjs buffer size " +
        this._sock.rQlen,
    );
              ...
              try {
                if (navigator.clipboard && navigator.clipboard.writeText) {
                  navigator.clipboard.writeText(textdata).then(
                    ...
                    (err) => {
                      Log.Error(
                        "Error writing text to client clipboard: " + err,
                      );
                    },
                  );
                }
  ...
  _handle_server_stats_msg() {
    ...
    Log.Debug(text);
    this.dispatchEvent(
      new CustomEvent("bottleneck_stats", { detail: { text: text } }),
    );
  ...
  _handleUserJoin() {
    ...
    const text = this._sock.rQshiftStr(length);
    this.dispatchEvent(
      new CustomEvent("sharedSessionUserJoin", { detail: text }),
    );
    return true;
  }
  ...
  _handleUserLeft() {
    ...
    const text = this._sock.rQshiftStr(length);
    this.dispatchEvent(
      new CustomEvent("sharedSessionUserLeft", { detail: text }),
    );
    return true;
  }
    ...
    switch (msgType) {
      case 0: // FramebufferUpdate
        ...
        if (this._trackFrameStats) {
          RFB.messages.sendFrameStats(
            this._sock,
            this._display.fps,
            this._display.renderMs,
          );
          this._trackFrameStats = false;
        }
  ...
  _sendUdpUpgrade() {
    if (
      this._transitConnectionState == this.TransitConnectionStates.Upgrading
    ) {
      return;
    }
    ...
    peer
      ...
      .then(function () {
        ...
        const str = Uint8Array.from(
          Array.from(peer.localDescription.sdp).map((letter) =>
            letter.charCodeAt(0),
          ),
        );
  ...
  _sendUdpDowngrade() {
    this._changeTransitConnectionState(
      this.TransitConnectionStates.Downgrading,
    );
          ...
          peer
            ...
            .catch(
              function (err) {
                ...
                this._changeTransitConnectionState(
                  this.TransitConnectionStates.Failure,
                );
                this._udpConnectFailures++;
              }.bind(this),
            );
  ...
  _handleDisconnectNotify() {
    ...
    if (
      reasonLength > 0 &&
      this._sock.rQwait("DisconnectNotify reason", reasonLength, 8)
    ) {
      return false;
    }
  ...
  _handleExtendedDesktopSize() {
    ...
    for (let i = 0; i < numberOfScreens; i += 1) {
      ...
      Log.Debug(
        `Server reported screen ${sI} with resolution ${w}x${h} at ${x}x${y}`,
      );
    }
    ...
    if (
      this._display.screens.length > 1 &&
      this._forceFullFrameUpdateAfterResize
    ) {
    ...
    try {
      if (
        ...
        if (this._transitConnectionState == this.TransitConnectionStates.Udp) {
          Log.Warn(
            "Implicit UDP Transit Failure, TCP rects recieved while in UDP mode.",
          );
        ...
        if (this._useUdp) {
          if (this._udpConnectFailures < 3 && this._udpTransitFailures < 3) {
            ...
            Log.Warn(
              "UDP connection failures exceeded limit, remaining on TCP transit.",
            );
          }
        }
      } else if (
        this._transitConnectionState == this.TransitConnectionStates.Downgrading
      ) {
        this._display.clear();
        this._changeTransitConnectionState(this.TransitConnectionStates.Tcp);
      }
