...
const UI = {
  ...
  initSettings() {
    ...
    if ("getScreenDetails" in window) {
      document
        .getElementById("noVNC_auto_placement_option")
        .classList.add("show");
    }
    ...
    let qualityDropdown = document.getElementById(
      "noVNC_setting_video_quality",
    );
    ...
    if (
      WebUtil.isInsideKasmVDI() &&
      !WebUtil.getConfigVar("show_control_bar")
    ) {
      UI.initSetting("clipboard_up", true);
      UI.initSetting("clipboard_down", true);
      // Enable seamless clipboard by default unless Firefox/Safari
  ...
  addKeyboardControlsPanelHandlers() {
    ...
    interact(".keyboard-controls .button.ctrlaltdel").on(
      "tap",
      UI.sendCtrlAltDel,
    );
  },
  ...
  addSettingsHandlers() {
    ...
    document
      .getElementById("noVNC_auto_placement")
      .addEventListener("change", UI.setAutoPlacement);
  ...
  isControlPanelItemClick(e) {
    if (!e?.target?.classList || !e?.target?.parentNode) return false;
  ...
  openControlbar() {
    ...
    if (WebUtil.isInsideKasmVDI()) {
      try {
        parent.postMessage(
          { action: "control_open", value: "Control bar opened" },
          "*",
        );
      } catch (e) {
  ...
  closeControlbar() {
    ...
    if (WebUtil.isInsideKasmVDI()) {
      try {
        parent.postMessage(
          { action: "control_close", value: "Control bar closed" },
          "*",
        );
      } catch (e) {
  ...
  updateSetting(name) {
    ...
    if (ctrl.type === "checkbox") {
      ...
      let value_label = document.getElementById(
        "noVNC_setting_" + name + "_output",
      );
      ctrl.value = value;
      if (value_label) {
  ...
  connect(event, password) {
    ...
    if (WebUtil.isInsideKasmVDI()) {
      ...
      UI._sessionTimeoutInterval = setInterval(function () {
        if (UI.rfb) {
          const timeSinceLastActivityInS =
            (Date.now() - UI.rfb.lastActiveAt) / 1000;
    ...
    if (event.data && event.data.action) {
      ...
      switch (event.data.action) {
        ...
        case "control_displays":
          try {
            parent.postMessage(
              { action: "can_control_displays", value: true },
              "*",
            );
          } catch (e) {
  ...
  clipboardRx(event) {
    try {
      parent.postMessage(
        { action: "clipboardrx", value: event.detail.text },
        "*",
      ); //TODO fix star
    } catch (e) {
  ...
  toggleFullscreen() {
    if (WebUtil.isInsideKasmVDI()) {
      try {
        parent.postMessage(
          { action: "fullscreen", value: "Fullscreen clicked" },
          "*",
        );
      } catch (e) {
  ...
  displaysRefresh() {
    ...
    let screenPlan = UI.rfb.getScreenPlan();
    document.getElementById("noVNC_refreshMonitors_icon").style.transform =
      "rotate(" + rotation + "deg)";
  ...
  increaseCurrentDisplay(details) {
    ...
    const thisIndex = details.screens.findIndex(
      (el) => el === details.currentScreen,
    );
    if (max === 1) {
      return 0;
    }
  ...
  async addSecondaryMonitor() {
    ...
    const auto_placement = document.getElementById(
      "noVNC_auto_placement",
    ).checked;
  ...
  initMonitors(screenPlan) {
    ...
    if (showNativeResolution) {
      document
        .getElementById("noVNC_setting_enable_hidpi_option")
        .classList.add("show");
    } else {
      document
        .getElementById("noVNC_setting_enable_hidpi_option")
  ...
  draw() {
    ...
    for (var i = 0; i < monitors.length; i++) {
      ...
      ctx.fillText(
        UI.toFixed(m.w * scale, 2) + " x " + UI.toFixed(m.h * scale, 2),
        m.x + m.w / 2,
        m.y + m.h / 2,
      );
    }
  ...
  togglePointerLock() {
    if (!supportsPointerLock()) {
      UI.showStatus(
        "Your browser does not support pointer lock.",
        "info",
        1500,
        true,
      );
  ...
  showVirtualKeyboard() {
    ...
    if (UI.getSetting("virtual_keyboard_visible")) {
      document
        .getElementById("noVNC_keyboard_control_handle")
        .classList.add("noVNC_selected");
    }
  ...
  toggleCtrl() {
    ...
    document
      .querySelector(".keyboard-controls .button.ctrl")
  ...
  toggleWindows() {
    ...
    document
      .querySelector(".keyboard-controls .button.windows")
      .classList.toggle("selected");
  },
  ...
  toggleAlt() {
    ...
    document
      .querySelector(".keyboard-controls .button.alt")
      .classList.toggle("selected");
  },
  ...
  inputLockChanged(e) {
    ...
    if (e.detail.pointer) {
      ...
      UI.closeControlbar();
      UI.showStatus(
        "Press Esc Key to Exit Pointer Lock Mode",
        "warn",
        5000,
        true,
      );
      ...
      if (UI.rfb.pointerRelative) {
        UI.showStatus(
          "Game Mode paused, click on screen to resume Game Mode.",
          "warn",
          5000,
          true,
        );
  ...
  screenRegistered(e) {
    ...
    if (UI.rfb) {
      ...
      if (e && e.detail) {
        ...
        const current = screenPlan.screens.findIndex(
